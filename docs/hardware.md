...
M5Stack Unit ENV Pro, SKU U169, is supported as an external BME688 unit on the
Nesso N1 external sensor connectors.
...
On Nesso N1, `NessoN1Board::into_display_and_env()` enables the Grove sensor
rail, probes the Grove GPIO5/GPIO4 path first, and falls back to the
shared Qwiic/main I2C bus. `nesso::bsp::init_env_pro(...)` is the simpler
shared-bus helper when the application already owns the main I2C device.
